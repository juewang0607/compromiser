...
type Config struct {
    ...
    // SigningKey is the key used by the reciepient, shares private key 
    // exponent with the static key of this peer, required if part of the
    // handshake
    SigningKey *ecdsa.PrivateKey
    
    // VerifyingKey is the key used by the initiator, It is the Public key 
    // Corresponding to SigningKey, required if part of the handshake
    // handshake
...
func NewHandshakeState(c Config) (*HandshakeState, error) {
	...
	for _, m := range c.Pattern.ResponderPreMessages {
		...
		case !c.Initiator && m == MessagePatternS:
			hs.ss.MixHash(hs.s.Public)
            if c.Pattern.Name == "IKSign" {
              hs.ss.MixHash(hs.verify.X.Bytes())
              hs.ss.MixHash(hs.verify.Y.Bytes())
            }
		case !c.Initiator && m == MessagePatternE:
			hs.ss.MixHash(hs.e.Public)
		case c.Initiator && m == MessagePatternS:
...
func (s *HandshakeState) WriteMessage(out, payload []byte) ([]byte, *CipherState, *CipherState, error) {
    ...
    if addSign == true {
        sig, _ := ECDSASign(s.sign, s.sign.PublicKey.Curve, out) 
        out = append(out, sig...)
...
func (s *HandshakeState) ReadMessage(out, message []byte) ([]byte, *CipherState, *CipherState, error) {
    signlen := 0 
    signdata := make([]byte, len(message))
    copy(signdata, message)
    if s.shouldWrite {
	...
	for _, msg := range s.messagePatterns[s.msgIdx] {
		...
		case MessagePatternPSK:
			s.ss.MixKeyAndHash(s.psk)
        case MessagePatternSign:
            expected := int(message[len(message)-1]) + 1
			if len(message) < expected {
			    return nil, nil, nil, ErrShortMessage
			}
            signlen = expected
            sig := message[len(message) - signlen:len(message)-1]
            verifycheck := ECDSAVerify(s.verify, s.verify.Curve, signdata, sig)
            if verifycheck == false {
              return nil, nil, nil, nil
            }
		}
	}
    msglen := len(message) - signlen
    out, err = s.ss.DecryptAndHash(out, message[:msglen])
	if err != nil {
		s.ss.Rollback()
		if rsSet {
			s.rs = nil
